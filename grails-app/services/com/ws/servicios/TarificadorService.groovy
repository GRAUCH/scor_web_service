...
import servicios.ConsultaExpediente as ConsultaExpedienteGorm
import com.scor.comprimirdocumentos.ParametrosEntrada
import servicios.Expediente
import servicios.InformeExpedientes
import servicios.InformeCitasPorCia
import servicios.InformeActividadesPorCia
import servicios.ConsultaExpediente
...
class TarificadorService {
	...
	def obtenerTipoReconocomiento = { expediente ->
		...
		if (expediente.tipoRMedico){
			...
			if (expediente.tipoRMedico=="16a" || expediente.tipoRMedico=="16b")
				result="16"
			else
			if (expediente.tipoRMedico=="06a" || expediente.tipoRMedico=="06b")
				result="06"
			else
	...
	def obtenerNodoConsultaExpediente(String idExpediente){
		try{
			...
			def salida=grailsApplication.mainContext.soapClientAlptis.consultaExpediente(usuario,filtro)//(countryName:"Spain")
