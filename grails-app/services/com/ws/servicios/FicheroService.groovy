...
import com.ws.enumeration.CoberturasEnum
import com.ws.enumeration.DatosAlptisEnum
import com.ws.enumeration.DatosCajamarEnum
import com.ws.enumeration.DatosEnum
import com.ws.enumeration.DatosLifesquareEnum
import com.ws.enumeration.RespuestasEnum
...
class FicheroService {
	...
	def procesarRespuestasFichero(mapBenefit,peticion){
		def cadena=""
		def error=""
		def numeroRegistros=0

		try{
			//RECIBIMOS ALGO ASI: benefit_name=FALLECIMIENTO##benefit_capital=500.00
			mapBenefit.each{
				//TODO
				...
				valor.each{
					if (it.split("==").size()==2) {
						...
						throw new Exception("Falta algun valor en preguntas y respuestas.")
					}
				}
			...
			logginService.putError("procesarRespuestasFichero",error)
			return [cadena, error, 0]
		}
	}
	...
	def procesarPieDePagina(numRegistros){
		...
		try{
			...
			cadena=""
			error="Error al procesar pie de pÃ¡gina del fichero: "+e.getMessage()
	...
	def preparar(peticion) {
		try{
			...
			if(req.descartado == false) {
				...
				mapAux.each{
					if(it.key.equals("question")){
						//SEPARAMOS ALGO ASI: question_code=DI##data_type=1##answer=Respuesta 1%%question_code=DI2##data_type=2##answer=Respuesta 2
						def valores=separadorPorcentaje(it.value)
						def cont=0
						valores.each{
							mapRespuestas.put("RES"+cont,it)
							cont++
						}
					}else if(it.key.equals("benefit")){
						//SEPARAMOS ALGO ASI: benefit_name=FALLECIMIENTO##benefit_capital=500.00%%benefit_name=DEPENDENCIA##benefit_capital=800.00%%
						def valores=separadorPorcentaje(it.value)
						def cont=0
						valores.each{
						...
						//SEPARAMOS ALGO ASI: record=A##date=2002-05-30T09:00:00##comments=Comentarios
				...
				if(nombrePeticion.equals("AfiEscaUnderwrittingCaseManagementRequest") || nombrePeticion.equals("AlptisUnderwrittingCaseManagementRequest") || nombrePeticion.equals("LifesquareUnderwrittingCaseManagementRequest")){
					def errores=""
					def numeroRegistros=0
					//PROCESAMOS DATOS DEL FICHERO
					def datosFile=procesarDatosFichero(mapFinal,peticion,obtenerDatosEnum(nombrePeticion))
					ficheroFinal=ficheroFinal+datosFile[0]
					errores=errores.concat(datosFile[1])
					//PROCESAMOS RESPUESTAS
					def respuestasFile=procesarRespuestasFichero(mapRespuestas,peticion)
					ficheroFinal=ficheroFinal+respuestasFile[0]
					errores=errores.concat(respuestasFile[1])
					...
					numeroRegistros=datosFile[2]+coberturasFile[2]+respuestasFile[2]

					//RETORNAMOS NOMBRE DEL FICHERO, DATOS DEL FICHERO Y ERRORES SI TUVIERA
					return [
						procesarNombreFichero(nombrePeticion.substring(0,4).toUpperCase()),
						ficheroFinal,
						errores,
						numeroRegistros
					]
			...
			return [
				null,
				null,
				"Error: No hay datos para tramitar en peticion: "+peticion+"\n",
				null
			]
		}
	}
	...
	def procesarLineaFichero (mapFinal,enumerado,peticion){
		...
		//SE TRATA FECHA NACIMIENTO
		if(valor.equals("ffnaci")){
			if(valorF.equals("2")) {
				valorF = "SOLICITUD"
				diferenciaBlancos = tamanioMaximo-valorF.trim().length()
			}
		...
		//SE TRATA TIPO DE SERVICIO
		if(valor.equals("zprumd")){
			if(valorF.equals("B")) {
				valorF = "637"
				diferenciaBlancos = tamanioMaximo-valorF.trim().length()
			}
		...
		try{
			...
			if(!valor.contains(',') && !valor.contains('.')){
				...
				if(aux[1].length()>=2){
					...
					//SI SOLO HAY UN DECIMAL AÃ‘ADIMOS LOS 0 RESTANTES
	...
	def procesarPeticion(peticiones) {
		...
		try{
			...
			error="Error en el metodo en procesar peticiÃ³n"+e
	...
	def obtenerDatosEnum = { nombrePeticion ->
		def result
