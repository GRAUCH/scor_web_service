...
<html>
    <head>
        ...
        <title>Editando aviso: ${avisoInstance}</title>
    </head>
    <body>

        <div class="body">
            <h1>Editando aviso: ${avisoInstance}</h1>
            ...
            <g:form method="post" >
                ...
                <g:hiddenField name="version" value="${avisoInstance?.version}" />
                <div class="dialog">
                    <table>
                        <tbody>
                         ...
                         --!>
                            ...
                            <tr class="prop">
                                ...
                                <td valign="top" class="value ${hasErrors(bean: avisoInstance, field: 'destinatarios', 'errors')}">
                                   <!-- <g:select name="destinatarios" from="${com.scortelemed.Destinatario.list()}" optionKey="id" value="${avisoInstance?.destinatarios?.id}"  />
                                    -->
                                    <ul>
										<g:each var="destino" in="${listaFinal}">
											<li>${destino.key.encodeAsHTML()}
												<g:checkBox name="DE_${destino.key.id}" value="${destino.value}"/>
											</li>
										</g:each>
									</ul>
                                </td>
                ...
                <div class="buttons">
                    ...
                    <span class="button"><input class="cancel" type="button" value="Cancelar" onclick="document.location='${createLinkTo(dir:'/aviso/show/' +  avisoInstance?.id)}'"/></span>                			
                </div>
            </g:form>
        </div>
    </body>
