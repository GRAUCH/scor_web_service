					...
					<tbody aria-relevant="all" aria-live="polite" role="alert">
						<g:each in="${elementos}" var="elemento" status="i">
							<tr class="\${(i % 2) == 0 ? 'gradeA even' : 'gradeA odd'}">
								...
								<td><g:if test="${elemento?.candidate?.birth_date != null}">
								...
								<td><g:if test="${elemento?.request_Data?.date != null}">
										<g:formatDate format="yyyy-MM-dd"
								...
								<td>
									${elemento?.policy?.BasicPolicyGroup?.policy_number}
								</td>
								<td>
									${elemento?.policy?.BasicPolicyGroup?.document_type}
								</td>
								<td>
